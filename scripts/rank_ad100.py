...
def main():
    ...
    if asa_map is not None:
        A(f"> **Primary sort key: ASA_Adj (deterministic computation, rubric v{rubric.get('version')}; `--rubric` re-scores historical subscores with a different rule set).** ASA_SelfReport is the CDA self-reported score, shown for display only.")
    else:
        A("> Primary sort key: CDA self-reported ASA_Score (no subscores in this run).")
    if mech_bonus:
        A(f"> Mechanism bonus (rubric mechanism_bonus): {', '.join(f'{k} +{v}' for k, v in mech_bonus.items())} —— sort key = ASA_Adj + bonus.")
    if pattern_bonus:
        for pb in pattern_bonus:
            A(f"> Pattern bonus (rubric pattern_bonus): {pb['label']} —— candidates whose Material_Name matches `{pb['regex']}` get +{pb['bonus']}.")
    A(f"> Source files: {', '.join(sources)}")
    A("")
    A(f"- Total candidates: **{len(ranked)}** ({len(unparsed)} lines unparsed; plus {prose} CDA prose-noise lines, not counted)")
