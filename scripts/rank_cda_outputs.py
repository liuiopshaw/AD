...
def main():
    ...
    if asa_map:
        ...
        out.append("> ASA_SelfReport is the CDA self-reported ASA_Score, shown for display only and not used for sorting. Subscores come from the JSON tail of the raw APA/EPA/BSA/MMA outputs (mechanically extracted by scripts/extract_subscores.py).")
    else:
        out.append("> All fields are preserved **verbatim** from the raw CDA agent output, sorted only by ASA score descending, with no content rewriting, cleaning, or curation.")
