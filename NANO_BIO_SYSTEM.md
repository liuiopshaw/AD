...
> 修复: adapter 切换 502/503 根因（见第七节问题 2）、task_100 脚本重复执行、batch_eval 端点错误
...
```powershell
python E:\Cu-agent\scripts\rank_cda_outputs.py R02   # 指定时间戳
...
```powershell
python E:\Cu-agent\scripts\formula_lookup.py R03
```
...
### 问题 5: task_100_materials.py 管道执行两遍 【已修复】
...
### 问题 6: batch_eval_100.py 调用不存在的端点 【已修复】
...
### 问题 7: Materials Project 工具链从未工作过 【部分修复】
...
- **待办**: ~~在 `.env` 填入有效 32 位 `MATERIALS_PROJECT_API_KEY`~~ 【已完成】MP 数据源已启用并验证（30/31 化合物解析，含 mp-id 溯源；CuCo3O4 两个库均无收录，为模型虚构化学式的诚实暴露）。PubChem 兜底通道保留
...
| CDA 实现计划 | `E:/Cu-agent/docs/superpowers/plans/-cda-implementation.md` |
...
- **实测（R03）**：Cu∩两者兼备 = **20 个**（即设计的 20/100=20%，实际总产出 115 中占 17%），Cu 总量 45 与配额一致；化学式列完整（CuO、Cu2O、Cu-N4、CuFe2O4 等）。

### 配体字段（ 第三轮）：小尺寸材料必须带稳定化配体
...
- **实测（R04）**：配体列全覆盖（PEG/glutathione/citrate/tannic acid/BSA 等有机配体，SAC 载体 CeO2/g-C3N4/ZIF-8）；配体查证命中 glutathione(CID:124886)、citrate(CID:31348)、tannic acid(CID:16129778)、ZIF-8(CID:15245636)；Cu∩两者兼备 22 (22%)。

### 补充（ 第四轮）：类别列回归 + Cu 最高频约束
...
- **实测（R04，无需重跑）**：两者兼备 34 个中 Cu×22 已为最高频（Fe×7、Ce×5、Pt×5、Ni×4、Au×1）,✅ 判定通过。

### 对照实验：原始基座 vs LoRA 智能体管道
...
- **结果**（`outputs/base_task100_R06.txt`，182 行原始输出）vs LoRA 管道（R05）：
...
- **实测（R07）**：全榜元素 **Fe×24、Cu×21**、Zn×13、Mo×13——Cu 居第二，与 Fe 同属第一梯队，分布自然；Cu 基材料 21 (18%)；Cu∩两者兼备 20（配额命中）；两者兼备中 Cu×20 仍为最高频 ✅。

---

## 十二、扩域重构 Phase 0:契约与地基
...
## 十三、扩域重构 Phase 1:文献索引、配比采样与数据质量闸
...
## 十四、扩域 Phase 2 Tier 1:运行时查证工具 + 泛化查证脚本
...
**已知环境问题**: 实测 PubChem 对本机 python-requests(urllib3 2.6.3)流量持续返回 503 PUGREST.ServerBusy,而同一 URL 用 curl/urllib/httpx 均 200——疑似 PubChem 侧按 TLS 指纹限流,非代码问题(既有 `pubchem_tool.py` 同样受影响);compound_lookup 的 PubChem 支路会优雅降级(NO MATCH + raw 落盘错误),SMILES 验证由 ChEMBL 兜底,建议后续观察或为重试加备用 HTTP 客户端。

## 十五、扩域 Phase 1b:Cu×菌群交叉文献补充与配额口径放宽
...
- **结果(run_R11)**:113 候选,子分提取 missing=4;种类 nano 57%/small_molecule 29%/biologic 10%/other 2%;**AD 机制 gut_microbiome_axis 30% 居首**(无 Cu 配额下肠-脑轴仍主导——训练数据富集内化迹象);Top3:LPS 中和抗体、FMT、铜卟啉。
...
**AD100 完整重跑(run_R12)**:124 候选,子分提取 102×5 轴 missing=0;种类 small_molecule 61%/nano 27%/biologic 11%;靶点 CNS 59%;机制 neuroprotection 31% 居首、gut_microbiome_axis 21%;枚举外仅 3 条(较上轮 28 条大幅改善)。新评分下 Top 全为递送优化型(外泌体包裹/水凝胶负载)——delivery 30% 权重导向符合预期。
...
## 二十一、AD100 r2:四项用户要求与三轮专家补救(~08-01)
...
### AD100 r2 数据库查证结果(compound_lookup,run_R13)

63/141 映射(其余为 other/名称不可查的复合物):① **旗舰家族全部可验证**——Cu 核 mp-30、β-CD CID:444041(C42H70O35)、γ-CD CID:5287407(C48H80O40),MP+PubChem 混合源命中;② 小分子 PubChem 名称解析准确(resveratrol CID:445154、curcumin CID:969516、quercetin CID:5280343、oleuropein CID:5281544),模型自报化学式与 PubChem 分子式一致;③ **生物制剂 UniProt 全为诚实 NA——上轮 P00001 型幻觉绝迹**(降幻觉铁律生效);SMILES 同为 NA(设计如此,由查证支路事后解析)。map json: compound_map_R13.json。

## 二十二、r3 去重修复与基座对照

**r3(run_R14)**:种子改 per-prompt 哈希(base+crc32(prompt),同 prompt 复现/异 prompt 去相关,已验证)+ CDA 跨批排除名单。重复行 89 中仅 10(r2 大量×4);**槲皮素旗舰家族占前 6**(自发演化为"抗氧化剂+槲皮素"双负载变体);Top20 菌群 20/20。CDA 散文噪声行 118 条单列统计(不计未解析);管道喂专家前过滤无管道符行(此前喂入产生 179 幻影材料名)。

**基座对照(run_R15,--base 全流程同提示词同种子)**:
...
run_R16:`pipeline_config_native.json`(4×25 全中性 focus,TOA 中性目标,无任何旗舰/菌群/Cu 导向)+ `--base` + 排名用 `asa_rubric.nobonus.json`(v0.3 权重,无机制/模式加分)。仅保留任务契约(四维分类枚举)与卫生规则(反幻觉 NA、跨批排除名单、温度/种子同 r3)。
...
## 二十四、评分标准 v0.4:生产质控轴(,评分标准/标准.md)
...
**结果(run_R17)**:旗舰家族 **ASA_Adj 真实分 8.55-8.66**(v0.2 时代 3.4-3.9)——manufacturability 9.0、biosafety 9.3-9.8、Cj 回升 0.92-0.96;**Top 10 全部为 CuNC@CD-槲皮素家族**(β-CD-quercetin 第 5、α 第 6、γ 第 7,EGCG/白藜芦醇/coQ10/阿魏酸双负载占 1-4),Top20 菌群机制 20/20。槲皮素旗舰的高排名现在主要来自真实评分(8.6)+ 透明加分(+6.0)的叠加,加分占比已从"全部"降为约 40%。
...
**结果(run_R18)**:**混合产物 0 条**(上版 43 条);**Top 10 全部为 β-CD/HP-β-CD 保护 Cu 纳米团簇**(CuNC@beta-CD_2nm_Cu 居首,真实 ASA_Adj 8.57-8.66 + 加分 6.0);子分提取 missing=0;唯一候选 94/119。
...
**结果(run_R19)**:134 行归并为 **82 个基础治疗剂家族**;**CuNC@beta-CD 以单一条目居榜首**(ASA_Adj 8.549 + 加分 6.0),CuO@HP-beta-CD 第 2、CuNC@HP-beta-CD 第 3;混合产物 0 条;子分 107×5 轴完整。

## 二十七、配额版 AD100

配置:批1 旗舰菌群纳米 25 / 批2 纳米 25 / 批3 小分子 30 / 批4 生物制剂 20,每批强制 Drug_Type(前次超时 run_R20 放弃续跑,全量重跑)。

**结果(run_R21)**:**CuNC@beta-CD 单条居首**(8.549+6.0);混合产物 0;93 家族/108 行。种类分布 nano 66 (61%) / 小分子 30 (27%) / 生物制剂 12 (11%),配额 50/30/20 的偏差来源:批2 超产(42/25);**批3 因 UniProt 失控幻觉("P0000…"数百个零)烧光 token 零产出**,批4 的小分子行补上了小分子配额(30/30 精确);批4 生物制剂 12/20 不足。
...
**结果(run_R22)**:**CuNC@beta-CD-quercetin 居榜首**(composite,真实分 7.46+加分 6.0);composite 24 条、三元违规 0;真实误分类 14 条(全部来自"纯纳米"批——纳米文献里"载体@包覆-负载"式命名本来就是 nanoformulation,模型的文献先验与 composite 规则冲突,反例加固提示词未见改善 12→14)。**结论:此规则的提示词纠偏已到天花板,剩余误分类需 Phase 5 训练数据层解决或展示层归一(待用户决策)。** 子分提取 79 条×5 轴 missing=0。

## 二十九、四类回归:复合归纳米+两组分上限
...
**结果(run_R23)**:**CuNC@beta-CD-quercetin 以 nano_formulation 居榜首**(8.51+6.0,UniProt 字段记负载名 quercetin);**超两组分违规 0**;composite 类别噪音消失。残留小瑕疵:1 条名为 NA 的废行(small_molecule,解析未拒);子分 76 条×轴完整。
...
**结果(run_R24,批3 经 redo)**:小分子 51 条全部实名(Metformin/Galantamine/Rivastigmine/Memantine/Donepezil/Sulforaphane…);分布 nano 45%/SM 38%/bio 15%(配额 50/30/20,SM 因 redo 批+批4 双重产出略超);**CuNC@beta-CD-quercetin 居首**;两组分违规 0;子分 122 条×5 轴完整。

## 三十一、当前构建的基座对照(,run_R25)
